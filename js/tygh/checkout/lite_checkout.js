(function (_, $) {
  ...
  var methods = {
    ...
    bindAbandonedCartFieldsSaving: function bindAbandonedCartFieldsSaving() {
      ...
      var shortDebounceUpdateCustomerInfo = methods.debounceUpdateCustomerInfo(addressFields, SHORT_REQUEST_DELAY);
      ...
      $('[data-ca-lite-checkout-auto-save="true"]', $checkout).on('focusout', shortDebounceUpdateCustomerInfo);
    },
    ...
    isValidActiveShipping: function isValidActiveShipping() {
      return !$shippingMethods.find('[data-ca-lite-checkout-element="shipping-method"]' + '[data-ca-lite-checkout-shipping-method-disabled="true"]:checked').length;
    },
    ...
    initPlainCityInput: function initPlainCityInput() {
      ...
      $country.on('change', function (e, extra) {
        ...
        if (methods.getElement('state', true).length) {
          $states.focus();
        } else {
          $city.focus();
        }
