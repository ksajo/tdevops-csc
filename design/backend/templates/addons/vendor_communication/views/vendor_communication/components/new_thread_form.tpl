<div id="new_thread_dialog_{$object_id}">
    <form action="{"vendor_communication.create_thread?communication_type={"Addons\\VendorCommunication\\CommunicationTypes::VENDOR_TO_ADMIN"|enum}"|fn_url}" method="post" class="form-horizontal form-edit add_message_form" name="add_thread_form_{$object_id}" id="new_thread_form_{$object_id}">
        <input type="hidden" name="result_ids" value="new_thread_message_{$object_id},threads_table" />
        <input type="hidden" name="redirect_url" value="{"vendor_communication.threads?communication_type={"Addons\\VendorCommunication\\CommunicationTypes::VENDOR_TO_ADMIN"|enum}"|fn_url}" />
        <input type="hidden" name="thread[object_type]" value="{$object_type}" />
        <input type="hidden" name="thread[object_id]" value="{$object_id}" />
        <input type="hidden" name="thread[communication_type]" value="{"Addons\\VendorCommunication\\CommunicationTypes::VENDOR_TO_ADMIN"|enum}" />
        ...
        <div id="new_message_{$thread_id}" class="add_message_form--wrapper">
            ...
            <div class="control-group">
                <label for="thread_message_{$object_id}" class="control-label vendor-communication-add-message__title cm-required">
                    {if $object.object_type == $smarty.const.VC_OBJECT_TYPE_COMPANY}
